...
void setup(){
  ...
  player = minim.loadFile("stayin-alive.mp3");
...
void draw() {
  ...
  IntVector userList1=new IntVector();
  kinect.getUsers(userList1);

  for(int i = 0; i < userList1.size(); i++){

    int userId1 = userList1.get(i);
    println("DEBUG TrackingSkeleton: "+kinect.isTrackingSkeleton(userId1));
   
    
    if (kinect.isTrackingSkeleton(userId1)) {
        
      //check to see if the user
      //is in the pose
     println("DEBUG: POSE CHECK POSE CHECK POSE CHECK: "+pose.check(userId1));
      if (pose.check(userId1)) {

        //if the are, set the color white
        stroke(255);
        ...
        if (!player.isPlaying()) {
          ...
          //otherwise set the color to red and don't start the song
          stroke(255, 0, 0);
        }
